...
function closeMobileDrawer(onClosed) {
  const drawer = document.getElementById('mobile-drawer');
  const backdrop = document.getElementById('drawer-backdrop');
  if (!drawer) return;
...
(async function AppInit() {
  ...
  document.getElementById('md-outline')?.addEventListener('click', () => {
    closeMobileDrawer(() => Outline.open());
  });
  ...
  const _iosBanner = document.getElementById('ios-install-banner');
  function _syncIOSBannerHeight() {
    document.documentElement.style.setProperty('--ios-banner-height', _iosBanner && !_iosBanner.hasAttribute('hidden')
      ? `${_iosBanner.offsetHeight}px`
      : '0px');
  }
  if (_isIOS && !_isStandalone && !_iosBannerDismissed) {
    _iosBanner?.removeAttribute('hidden');
    _syncIOSBannerHeight();
    requestAnimationFrame(_syncIOSBannerHeight);
  } else {
    _syncIOSBannerHeight();
  }
  document.getElementById('ios-banner-close')?.addEventListener('click', () => {
    _iosBanner?.setAttribute('hidden', '');
    sessionStorage.setItem('ios_banner_dismissed', '1');
    _syncIOSBannerHeight();
  });
  window.addEventListener('resize', _syncIOSBannerHeight);
