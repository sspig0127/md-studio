...
| 新手導覽 Onboarding Tour | 首次開啟時逐步介紹各功能與位置，可隨時跳過或重播 | ✅ 已完成（2026-03-13）|
...
目前手機導覽（6 步）未包含大綱面板說明。計畫新增第 7 步：

| 步驟 | 目標元素 | 說明重點 |
|------|---------|---------|
| 7（新增）| `.status-bar` | 說明右下角 ○ 按鈕，搭配 **箭頭圖示指向** `#btn-outline` 位置 |

**設計方向：**
- Bubble 描述文字引導使用者注意狀態列右下角
- 在 Bubble 內加入 `↘` 箭頭符號（或 CSS 箭頭元素），視覺指向 `#btn-outline`
- 同步套用 `pulseTarget: '#btn-outline'` 脈衝動畫（已於桌機版實作）
- 可考慮 Bubble 位置改為固定於按鈕上方（而非底部 100px）以更靠近目標
...
*最後更新：2026-03-14（新手導覽大綱按鈕改善、手機版待辦規劃）*
